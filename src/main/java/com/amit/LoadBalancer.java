...
public class LoadBalancer extends Thread {
    ...
    private int numberOfBackStreamConnections = 0;
    UpStreamManagement upStreamManagement;
    private int loadBalancerPort = 80;
    private String loadBalancerIP = "127.0.0.1";
    private int loadBalancerBufferSize = 1024;

    private static Logger logger = LogManager.getLogger(LoadBalancer.class);

    LoadBalancer(UpStreamManagement upStreamManagement, Configuration appConfig) {
        this.upStreamManagement = upStreamManagement;
        this.loadBalancerPort = appConfig.getInt("lb_port");
        this.loadBalancerIP = appConfig.getString("lb_ip_address");
        this.loadBalancerBufferSize = appConfig.getInt("lb_buffer_size");
    }
    ...
    public void run() {
        try {
            ...
            logger.info("Binding load balancer at interface " + this.loadBalancerIP + " and port " + this.loadBalancerPort);
            serverSocket.bind(new InetSocketAddress(this.loadBalancerIP, this.loadBalancerPort));
            serverSocket.configureBlocking(false);

            HashMap<String, Object> attr = new HashMap<>();
            attr.put("channel_type", "down_stream");
            attr.put("overload_connection", "");
            attr.put("connection_last_used_time", java.time.Instant.now().getEpochSecond());
            ...
            while (true) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (selectionKey.isValid() && selectionKey.isAcceptable() && channelType.equals("down_stream")) {
                        ...
                        this.numberOfConnection = this.numberOfConnection + 1;
                        downstreamConnection.configureBlocking(false);

                        HashMap<String, Object> x = new HashMap<>();
                        x.put("channel_type", "down_stream");
                        x.put("overload_connection", "");
                        x.put("connection_last_used_time", java.time.Instant.now().getEpochSecond());
                    ...
                    if (selectionKey.isValid() && selectionKey.isReadable() && channelType.equals("down_stream")) {
                        ...
                        ByteBuffer buffer = ByteBuffer.allocate(loadBalancerBufferSize);
                        ...
                        try {
                            ...
                            if (downstreamConnection.read(buffer) < 0) {
                                ...
                                long lastUsed = (long) yy.get("connection_last_used_time");
                                ...
                                HashMap<String, Object> yy = (HashMap<String, Object>) selectionKey.attachment();
                                yy.put("connection_last_used_time", java.time.Instant.now().getEpochSecond());
                                ...
                                attrClient.put("connection_last_used_time", java.time.Instant.now().getEpochSecond());
